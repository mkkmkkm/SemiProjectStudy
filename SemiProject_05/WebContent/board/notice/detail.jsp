...
<%
   ...
   */
   ...
   if(!keyword.equals("")){
      ...
      if(condition.equals("title_content")){//제목 + 내용 검색인 경우
         ...
         dto=NoticeDao.getInstance().getDataTC(dto);
      }else if(condition.equals("title")){ //제목 검색인 경우
         dto.setTitle(keyword);
         dto=NoticeDao.getInstance().getDataT(dto);
      }else if(condition.equals("writer")){ //작성자 검색인 경우
         dto.setWriter(keyword);
         dto=NoticeDao.getInstance().getDataW(dto);
      } // 다른 검색 조건을 추가 하고 싶다면 아래에 else if() 를 계속 추가 하면 된다.
   }else{//검색 키워드가 넘어오지 않는다면
      dto=NoticeDao.getInstance().getData(dto);
   }
   ...
   */
   ...
   List<noticeCommentDto> commentList=
         noticeCommentDao.getInstance().getList(commentDto);
   
   //원글의 글번호를 이용해서 댓글 전체의 갯수를 얻어낸다.
   int totalRow=noticeCommentDao.getInstance().getCount(num);
   //댓글 전체 페이지의 갯수
...
<div class="container">
   ...
   <div class="comments">
      <ul>
         <%for(NoticeCommentDto tmp: commentList){ %>
   ...
   document.querySelector(".insert-form")
      .addEventListener("submit", function(e){
         ...
         if(!isLogin){
            ...
            //로그인 폼으로 이동 시킨다.
            location.href=
               "${pageContext.request.contextPath}/users/loginform.jsp?url=${pageContext.request.contextPath}/notice/detail.jsp?num=<%=num%>";
         }
      });
